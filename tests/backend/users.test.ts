  ...
  describe("GET /api/users/:id/services", () => {
    ...
    it("should return services for a provider", async () => {
      const res = await request(app).get(
        `/api/users/${providerUser.id}/services`
      );
      expect(res.status).toBe(200);
      expect(res.body.data.services).toBeDefined();
      expect(res.body.data.services.length).toBe(2);
      expect(res.body.data.services[0]).toHaveProperty("title");
    });

    it("should return 404 for a non-provider user", async () => {
      const res = await request(app).get(
        `/api/users/${clientUser.id}/services`
      );
    ...
    it("should return 404 for an invalid user ID", async () => {
      const res = await request(app).get(`/api/users/invalid-id/services`);
      expect(res.status).toBe(404);
      expect(res.body).toHaveProperty("error", "User not found");
    });
  });
